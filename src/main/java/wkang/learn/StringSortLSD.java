...
public class StringSortLSD {
    ...
    public static void LSDSort(String[] str, int W) {
        ...
        for (int d = W - 1; d >= 0; d--) {
            // 计算每个字符加一的出现频率
            for (int i = 0; i < N; i++) cnt[str[i].charAt(d) + 1]++;
            ...
            // 可以看到LSD为原地排序
            for (int i = 0; i < N; i++) strTmp[cnt[str[i].charAt(d)]++] = str[i];
            // 将临时数组copy回原数组
            for (int i = 0; i < N; i++) str[i] = strTmp[i];
            // 初始化cnt数组
